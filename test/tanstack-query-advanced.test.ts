...
import { createEdenTQ, createEdenTQUtils } from '../src/tanstack-query'
...
describe('EdenTQ Utils', () => {
    ...
    it('infers types correctly from eden instance (type inference fix)', () => {
        const queryClient = new QueryClient()
        const utils = createEdenTQUtils(eden, queryClient)

        const queryKey = utils.user({ id: '1' }).get.queryKey({ params: { id: '1' } })
        expect(Array.isArray(queryKey)).toBe(true)

        const queryOpts = utils.posts.get.queryOptions({ query: { limit: '10' } })
        expect(queryOpts.queryKey).toBeDefined()
        expect(typeof queryOpts.queryFn).toBe('function')

        const mutationOpts = utils.user.post.mutationOptions()
        expect(mutationOpts.mutationKey).toBeDefined()
